...
class govukSelectIntegrationSpec extends TemplateIntegrationSpec {
  ...
  @Inject private val default =
    Guice.createInjector().getInstance(classOf[uk.gov.hmrc.govukfrontend.views.html.examples.select.default])

  testRendering(GovukFrontend, "select", "default", default.f)
}
